...
<?php get_header('page'); ?>. Разница с обычным вызывом в том, что мы передаем в в функцию вызова аргумент идентифицирующий тот header, который нам нужен, а именно header-page.php. WP сам понимает по второй части названия файла, что он относится к нужному нам header.
